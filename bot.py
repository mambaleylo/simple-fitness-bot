...
async def send_admin_panel(target, edit=False):
    text = (
        ...
        "📅 <b>Добавить тренировку месяца</b> — встаёт в очередь и автоматически отправляется подписчикам в пн/ср/пт в 20:00\n"
...
async def adm_help(callback: types.CallbackQuery):
    ...
    text = (
        ...
        "Каждый пн/ср/пт в 20:00 бот берёт следующую из очереди и рассылает подписчикам. "
...
async def adm_add_weekly_start(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        "в ближайший день по расписанию (пн/ср/пт в 20:00).\n\n"
        "Введи <b>название</b> тренировки:",
        reply_markup=cancel_keyboard(), parse_mode="HTML"
    )
    await callback.answer()
...
async def send_tomorrow_workout():
    """
    По расписанию (ПН/СР/ПТ, 20:00): вечером отправляем подписчикам
...
async def main():
    ...
    # Джобы отправки тренировок по расписанию (пн/ср/пт в 20:00)
