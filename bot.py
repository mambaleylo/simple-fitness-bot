...
async def send_admin_panel(target, edit=False):
    text = (
        ...
        "📅 <b>Добавить тренировку месяца</b> — встаёт в очередь и автоматически отправляется подписчикам в вс/вт/чт/пт в 21:00\n"
...
async def adm_help(callback: types.CallbackQuery):
    ...
    text = (
        ...
        "Каждый вс/вт/чт/пт в 21:00 бот берёт следующую из очереди и рассылает подписчикам. "
...
async def adm_add_weekly_start(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        "в ближайший день по расписанию (вс/вт/чт/пт в 21:00).\n\n"
        "Введи <b>название</b> тренировки:",
        reply_markup=cancel_keyboard(), parse_mode="HTML"
    )
    await callback.answer()
...
async def send_tomorrow_workout():
    """
    По расписанию (ВС/ВТ/ЧТ/ПТ, 21:00): вечером отправляем подписчикам
...
async def remind_add_workout():
    """
    Напоминание в день тренировки в 12:00 — проверяем очередь.
...
async def main():
    ...
    # Джобы отправки тренировок по расписанию (вс/вт/чт/пт в 21:00)
    for day in SCHEDULE_DAYS:
        scheduler.add_job(
            send_tomorrow_workout, trigger="cron",
            day_of_week=day, hour=int(hour), minute=int(minute)
        )
        # Напоминание в тот же день в 12:00
        scheduler.add_job(
            remind_add_workout, trigger="cron",
            day_of_week=day, hour=12, minute=0
        )
